...
if(TRUE){
  ...
  obs_err_cov = matrix(c(0.05,0,0,100), nrow=2)
  ...
  for(ee in 1:length(epsvals)){
    ...
    for(ii in 1:ntrials){
      ground_truth[ee,ii,,] = sim_spring(
        ...
        k=k,m=m,g=g,epsilon = epsvals[ee], Q = sqrt(system_noise_var)
      )
      kalman_est = kalman_filter(ground_truth[ee,ii,,],
                                 Q=diag(c(0.000,system_noise_var)),
                                 k=4,g=g,m=m,R=obs_err_cov,dt=dt)
...
eval_ndcs = seq(100,500,by=20)
cvgs = reliability_plot(epsvals, ground_truth[,,1,eval_ndcs],
                        preds[,,eval_ndcs],
